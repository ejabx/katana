...
public:
	...
	/// If bCloseBox is specified, then the number of points is 17, which is needed
	/// to render a closed version of the box (using line strips).
	void getCorners( Point3 * corners, bool bCloseBox = false );
...
//
inline void AxisAlignedBox::getCorners( Point3 * corners, bool bCloseBox )
{
	corners[0] = m_minimum;										// Bottom Front Left
	corners[1] = Point3(m_minimum.x, m_maximum.y, m_minimum.z); // Top Front Left
	corners[2] = Point3(m_maximum.x, m_maximum.y, m_minimum.z); // Top Front Right
	corners[3] = Point3(m_maximum.x, m_minimum.y, m_minimum.z); // Bottom Front Right
	corners[4] = Point3(m_maximum.x, m_minimum.y, m_maximum.z); // Bottom Back Right
	corners[5] = m_maximum;										// Top Back Right
	corners[6] = Point3(m_minimum.x, m_maximum.y, m_maximum.z); // Top Back Left
	corners[7] = Point3(m_minimum.x, m_minimum.y, m_maximum.z); // Bottom Back Left

	if ( bCloseBox )
	{
		corners[ 8] = m_minimum;										// Bottom Front Left
		corners[ 9] = Point3(m_minimum.x, m_maximum.y, m_minimum.z);	// Top Front Left
		corners[10] = Point3(m_minimum.x, m_maximum.y, m_maximum.z);	// Top Back Left
		corners[11] = Point3(m_minimum.x, m_minimum.y, m_maximum.z);	// Bottom Back Left
		corners[12] = Point3(m_maximum.x, m_minimum.y, m_maximum.z);	// Bottom Back Right
		corners[13] = m_maximum;										// Top Back Right
		corners[14] = Point3(m_maximum.x, m_maximum.y, m_minimum.z);	// Top Front Right
		corners[15] = Point3(m_maximum.x, m_minimum.y, m_minimum.z);	// Bottom Front Right
		corners[16] = m_minimum;										// Bottom Front Left
	}
}
